...
(function ($) {
  ...
  $.extend($.reportGtstar, {
    ...
    //查看详细配置表
    detailsModal: function detailsModal(reportid) {
      //detailsModal
      var reportopt = {
    ...
    //删除配置表提示
    deleteModal: function deleteModal(reportid) {
      // :nth-child(2)")
    ...
    //更改表格的启用标识
    editEnableFlag: function editEnableFlag(reportid, status) {
      ...
      var opt = {
        ...
        callback: function callback(data) {
          ...
          $("tr[data-reportId='" + reportid + "'] td>button[value='editEnableFlag']").attr("onclick", "$.reportGtstar.editEnableFlag('" + reportid + "','" + status + "')").html(msg);
          // location.reload();
        }
      };
      common.sendAjax(opt);
          ...
          if (data) {
            ...
            if (data.exportFormat === 1) {
              ...
              if (exportsult.length === 0) {
                ...
                $("#exportpreview").prop("value", exportsult).removeAttr("style");
                // console.log("后缀名有设置");
              }
            } else {
    ...
    getReportView: function getReportView() {
      //基础报表数据绑定
      var opt = {
    ...
    //基础表获取指标项事件
    getcolumnsByreportId: function getcolumnsByreportId(rID) {
      var opt = {
    ...
    //移除指标更变表格事件
    deleteIndicatorToViewtable: function deleteIndicatorToViewtable(that) {
      delete indicatorData.reportindicatorConfig[$(that).val()];
    },
    groupFlagIndicatorToViewtable: function groupFlagIndicatorToViewtable(that) {
    ...
    updateIndicatorToViewtable: function updateIndicatorToViewtable(that) {
    ...
    //类型更新指标信息数据
    renewIndicatorToViewtable: function renewIndicatorToViewtable(that) {
    ...
    //指标到视图的方法
    indicatorToViewtable: function indicatorToViewtable() {
    ...
    //重置缓存的配置表全局数据
    restreportConfigData: function restreportConfigData() {
        ...
        if (indicatorFilter != 0) {
          ...
          if (indicatorFilter == "input") {
            ...
            if (filter.field === "neid") {
              var nidfilters = [];
              $('#nidmultiple option:selected').map(function (a, item) {
                return nidfilters.push(item.value);
              });
              // var idname=[];
  ...
  $(".exportoptdiv").on("change", ".exportopt", function () {
    ...
    if (reg.test(exportsult)) {
      ...
      $("#exportpreview").prop("value", "文件名不能包含\/\\:\*\?\"<>\|任一字符").prop("style", "color:red");
    } else {
