...
;(function($,window,document,undefined) {
	...
	TheTable.prototype = {
		...
		initTable: function () {
			...
			this.options.yKey['yKey'] && this.initColHeader(table);
		...
		initColHeader:function(table){
			this.readColLeafList();
			this.initColHeaderList();
		},
		readColHeaderJsonToLevelList:function (level) {
			var $this = this;//定义变量指向，循环中this指向会变化，无法正确引用
			$.each(this.options.colHeadJson, function (i, item) {
				if(item){
					...
					if (lvl == level) {
						...
						if($this.options.colConfig.width){
							optionList.push(["style","width:"+$this.readObjDymValue(item, $this.options.colConfig.width)]);
						}
						//判断是否叶子节点，进行不同的属性处理
						if(!trList[level - $this.options.colStartlay]){
							trList[level - $this.options.colStartlay]=[];
						}
						if (isleaf == 'N') {
							//如果 不是叶子节点，就需要读取子节点的数量，并且书写rowspan属性
							var col = $this.readLeafCount(code,colLeafList);
							optionList.push([$this.tablestr.rows, col == 0 ? 1 : col]);

							trList[level - $this.options.colStartlay].push($this.formatElementStr($this.tablestr.td, optionList, showname));
							for(var i=0;i<col-1;i++){
								trList[level - $this.options.colStartlay].push("");
							}
						} else {
						...
						//加入到headerList大家庭中
						colHeaderList[level - $this.options.colStartlay].push([code, $this.formatElementStr($this.tablestr.td, optionList, showname)]);
					}
				}
			});
		},
		initColHeaderList:function(){
			for (var lever = this.options.colStartlay; lever < this.options.colStartlay+this.options.colLaynum; lever++) {
				colHeaderList.push([]);
				this.readColHeaderJsonToLevelList(lever);
			}
		},
		readColLeafList:function() {
			var $this = this;//定义变量指向，循环中this指向会变化，无法正确引用
			$.each(this.options.colHeadJson, function (i, item) {
				if (item.isleaf == 'Y') {
					var leaf = [$this.readObjDymValue(item, $this.options.colConfig.treecode),
						...
						$this.readObjDymValue(item, $this.options.colConfig.columnValue)];
					colLeafList.push(leaf);

					colColumnValues.push(leaf[2]);
				}
			});
		},
		/**
		...
		writeTableData: function () {
			if(leafList.length>0){
				...
				var $this = this;
				if(!this.options.yKey['yKey'] || colColumnValues.length === this.options.dataJson.length){
					var trall = '';
					$.each(this.options.dataJson, function (i, item) {
						if(item){
							$this.resultData.push(item);
							trall += $this.createTrString(item,i);
							trcount++;
						}
					});
					...
					$('#'+this.options.tableid+'_'+this.tablestr.prefix+this.tablestr.body).append(trall);
				} else {
					var trall = [];
					var yKey = $this.options.yKey['yKey'];

					for(var i in this.options.dataJson) {
						var item = this.options.dataJson[i];
						if(item){
							if(trcount === colColumnValues.length){
								break;
							}

							var index = colColumnValues.indexOf(item[yKey]);
							if(index == -1) {
								continue;
							}
							$this.resultData[index] = item;
							trall[index] = $this.createTrString(item,index);
							trcount++;
						}
					}
					$('#'+this.options.tableid+'_'+this.tablestr.prefix+this.tablestr.body).append(trall.join(''));
				}
			}
		},
		/**
		 * 生成tr
		 */
		createTrString : function(item, trindex){
			var $this = this;
			var tdall = '';
			for(var level in trList) {
				tdall+=trList[level][trindex];
			}
			...
			for(var idx=0;idx<leafList.length;idx++) {
				...
				vlue = $this.handleValueZero(vlue);
			...
			return $this.formatElementStr($this.tablestr.tr,optr,tdall);
		},
		...
		initTableElement: function (table) {
			...
			table.append(this.formatElementStr(this.tablestr.head, [[this.tablestr.id,this.options.tableid+'_'+this.tablestr.prefix+this.tablestr.head]]));

			//生成tbody，如果不存在 generate body if not exists
			table.find(this.tablestr.body).length == 0 &&
			table.append(this.formatElementStr(this.tablestr.body, [[this.tablestr.id,this.options.tableid+'_'+this.tablestr.prefix+this.tablestr.body]]));
		...
		initHeaderList: function () {
			var $this = this;
			headerList.push([]);
			$this.options.colLaynum && headerList[0].push(['0',$this.formatElementStr($this.tablestr.th,[[$this.tablestr.rows,$this.options.laynum],[$this.tablestr.cols,$this.options.colLaynum]],$this.options.yKey.show)])
			for (var lever = this.options.startlay; lever < this.options.startlay+this.options.laynum; lever++) {
				if(lever !=this.options.startlay);{
					headerList.push([]);
				}
		...
		writeHeader: function (table,headerList) {
			if (headerList.length >= 0) {
				...
				$('#'+this.options.tableid+"_"+this.tablestr.prefix+this.tablestr.head).append(headtr);
				//防止之前是隐藏状态，在这里进行一次显示
				table.show();
			}
		},
		...
		formatElementStr: function (elmtname, propList, innerHtm) {
			...
			if (elmtname != null) {
				...
				innerHtm = $this.handleValueZero(innerHtm);
				return $this.tablestr.elmtstr.format(elmtname, propstr, innerHtm);
			} else {
				return '';
			}
		},
		handleValueZero:function(val){
			if(!this.zeroNumberShow){
				val = val || '';
			} else {
				if(val==null){
					val= '';
